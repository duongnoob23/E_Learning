// ExamWritingPart3Editor.jsx - Editor for Writing Part 3 (Write an essay based on a reading passage)
// Part 3: 1 question required
// Features: Preview, JSON Import

import { useCallback, useEffect, useRef, useState } from "react";
import "./ExamWritingPart3Editor.css";

const MAX_QUESTIONS = 1;
const MIN_QUESTIONS = 1;

function createEmptyQuestion() {
  const id = `writing_p3_q_${Date.now()}_${Math.random()
    .toString(36)
    .slice(2, 8)}`;
  return {
    id,
...
export default function ExamWritingPart3Editor({ questions = [], onChange }) {
  ...
  const mapExamDataToState = useCallback((examQuestions) => {
    ...
    return {
      id: q.question_id || q.id || `writing_p3_q_${Date.now()}`,
      question_number: q.question_number || 1,
      question_text: q.question_text || "",
      image_file: q.image_file || "", // Optional
    };
  ...
  const handleImportJSON = () => {
    ...
    const mapped = {
      id:
        q.question_id ||
        `writing_p3_q_${Date.now()}_${Math.random()
          .toString(36)
          .slice(2, 8)}`,
      question_number: q.question_number || 1,
      question_text: q.question_text || "",
  ...
  const sampleJSON = `[
  {
    "question_id": "writing_p3_q1",
    "question_number": 1,
    "question_text": "Write an essay based on a reading passage."
  }
]`;

  return (
    <div className="exam-writing-p3-editor">
      {/* Toolbar */}
      <div className="exam-writing-p3-editor__toolbar">
        <div className="exam-writing-p3-editor__toolbar-left">
          <button
            className="exam-writing-p3-editor__btn exam-writing-p3-editor__btn--preview"
            onClick={() => setShowPreview(!showPreview)}
          >
            {showPreview ? "✏️ Edit Mode" : "👁️ Preview"}
          </button>
          <button
            className="exam-writing-p3-editor__btn exam-writing-p3-editor__btn--import"
            onClick={() => setShowImportJSON(!showImportJSON)}
          >
            📝 Import JSON
          </button>
        </div>
        <div className="exam-writing-p3-editor__toolbar-right">
          <span className="exam-writing-p3-editor__info">
            Part 3: Write an essay based on a reading passage (1 question)
          </span>
        </div>
      </div>
      ...
      {/* Import JSON Modal */}
      {showImportJSON && (
        <div className="exam-writing-p3-editor__import-modal">
          <div className="exam-writing-p3-editor__import-content">
            ...
            />
            {jsonError && (
              <div className="exam-writing-p3-editor__error">{jsonError}</div>
            )}
            <div className="exam-writing-p3-editor__import-actions">
      ...
      {/* Preview Mode */}
      {showPreview ? (
        <div className="exam-writing-p3-editor__preview">
          <div className="exam-writing-p3-editor__preview-header">
            <h3>Part 3 - TOEIC Writing</h3>
          </div>

          <div className="exam-writing-p3-editor__preview-content">
            {/* Left: Question */}
            <div className="exam-writing-p3-editor__preview-question">
              <div className="exam-writing-p3-editor__preview-question-card">
                <h4>Question {currentQuestion.question_number}</h4>
                {currentQuestion.question_text && (
                  <div className="exam-writing-p3-editor__preview-text">
                    {currentQuestion.question_text}
                  </div>
                )}
                {currentQuestion.image_file && (
                  <div className="exam-writing-p3-editor__preview-image">
                    <img
            ...
            <div className="exam-writing-p3-editor__preview-answer">
              <div className="exam-writing-p3-editor__preview-answer-card">
                <div className="exam-writing-p3-editor__preview-answer-number">
                ...
                <textarea
                  ...
                  className="exam-writing-p3-editor__preview-notes"
                />
                <div className="exam-writing-p3-editor__preview-word-count">
                  Word count: {previewNotes.split(/\s+/).filter(Boolean).length || 0}
                </div>
              </div>
            </div>
          </div>
        ...
        /* Edit Mode */
        <div className="exam-writing-p3-editor__edit">
          {/* Question Form */}
          <div className="exam-writing-p3-editor__form">
            <div className="exam-writing-p3-editor__field">
              <label>
