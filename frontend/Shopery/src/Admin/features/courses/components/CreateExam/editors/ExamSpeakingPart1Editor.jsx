// ExamSpeakingPart1Editor.jsx - Editor for Speaking Part 1 (Read a text aloud)
// Part 1: 2 questions required
// Features: Preview, JSON Import, Tabs navigation

import { useCallback, useEffect, useRef, useState } from "react";
import "./ExamSpeakingPart1Editor.css";
...
export default function ExamSpeakingPart1Editor({ questions = [], onChange }) {
  ...
  const handleImportJSON = () => {
    ...
    const mapped = arr.map((q, idx) => ({
      id:
        q.question_id ||
        `speaking_p1_q_${idx}_${Date.now()}_${Math.random()
          .toString(36)
          .slice(2, 8)}`,
      question_number: q.question_number || idx + 1,
      question_text: q.question_text || "",
  ...
  return (
    <div className="exam-speaking-p1-editor">
      {/* Toolbar */}
      <div className="exam-speaking-p1-editor__toolbar">
        <div className="exam-speaking-p1-editor__toolbar-left">
          <button
            className="exam-speaking-p1-editor__btn exam-speaking-p1-editor__btn--preview"
            onClick={() => setShowPreview(!showPreview)}
          >
            {showPreview ? "✏️ Edit Mode" : "👁️ Preview"}
          </button>
          <button
            className="exam-speaking-p1-editor__btn exam-speaking-p1-editor__btn--import"
            onClick={() => setShowImportJSON(!showImportJSON)}
          >
            📝 Import JSON
          </button>
        </div>
        <div className="exam-speaking-p1-editor__toolbar-right">
          <span className="exam-speaking-p1-editor__info">
            Part 1: Read a text aloud ({currentQuestions.length}/{MAX_QUESTIONS} questions)
          </span>
        </div>
      </div>
      ...
      {/* Import JSON Modal */}
      {showImportJSON && (
        <div className="exam-speaking-p1-editor__import-modal">
          <div className="exam-speaking-p1-editor__import-content">
            ...
            />
            {jsonError && (
              <div className="exam-speaking-p1-editor__error">{jsonError}</div>
            )}
            <div className="exam-speaking-p1-editor__import-actions">
      ...
      {/* Preview Mode */}
      {showPreview ? (
        <div className="exam-speaking-p1-editor__preview">
          <div className="exam-speaking-p1-editor__preview-header">
            <h3>Part 1 - TOEIC Speaking</h3>
            <div className="exam-speaking-p1-editor__preview-nav">
              {currentQuestions.map((q, idx) => (
                <button
                  key={q.id}
                  onClick={() => setCurrentIndex(idx)}
                  className={`exam-speaking-p1-editor__preview-nav-btn ${
          ...
          <div className="exam-speaking-p1-editor__preview-content">
            {/* Left: Question */}
            <div className="exam-speaking-p1-editor__preview-question">
              <div className="exam-speaking-p1-editor__preview-question-card">
                <h4>Question {currentQuestion.question_number}</h4>
                {currentQuestion.question_text && (
                  <div className="exam-speaking-p1-editor__preview-text">
                    {currentQuestion.question_text}
                  </div>
                )}
                {currentQuestion.image_file && (
                  <div className="exam-speaking-p1-editor__preview-image">
                    <img
            ...
            <div className="exam-speaking-p1-editor__preview-answer">
              <div className="exam-speaking-p1-editor__preview-answer-card">
                <div className="exam-speaking-p1-editor__preview-answer-number">
                ...
                <textarea
                  ...
                  className="exam-speaking-p1-editor__preview-notes"
                />
                <button className="exam-speaking-p1-editor__preview-record-btn">
                  🎤 RECORD
                </button>
              </div>
            </div>
        ...
        /* Edit Mode */
        <div className="exam-speaking-p1-editor__edit">
          {/* Question Tabs */}
          <div className="exam-speaking-p1-editor__tabs">
            {currentQuestions.map((q, idx) => (
              <button
                key={q.id}
                onClick={() => setCurrentIndex(idx)}
                className={`exam-speaking-p1-editor__tab ${
                  currentIndex === idx ? "active" : ""
                }`}
              >
          ...
          {/* Question Form */}
          <div className="exam-speaking-p1-editor__form">
