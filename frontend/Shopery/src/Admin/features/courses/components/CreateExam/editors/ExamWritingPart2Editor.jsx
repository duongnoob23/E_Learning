// ExamWritingPart2Editor.jsx - Editor for Writing Part 2 (Write an essay)
// Part 2: 2 questions required, has image_file
// Features: Preview, JSON Import, Tabs navigation

import { useCallback, useEffect, useRef, useState } from "react";
import "./ExamWritingPart2Editor.css";
...
export default function ExamWritingPart2Editor({ questions = [], onChange }) {
  ...
  const mapExamDataToState = useCallback((examQuestions) => {
    ...
    const mapped = examQuestions.map((q, idx) => ({
      id: q.question_id || q.id || `writing_p2_q_${idx}_${Date.now()}`,
      question_number: q.question_number || idx + 1,
      question_text: q.question_text || "",
      image_file: q.image_file || "", // Part 2 has image
    }));
  ...
  const handleImportJSON = () => {
    ...
    const mapped = arr.map((q, idx) => ({
      id:
        q.question_id ||
        `writing_p2_q_${idx}_${Date.now()}_${Math.random()
          .toString(36)
          .slice(2, 8)}`,
      question_number: q.question_number || idx + 1,
      question_text: q.question_text || "",
  ...
  const sampleJSON = `[
  {
    "question_id": "writing_p2_q1",
    "question_number": 1,
    "question_text": "Write an essay based on the email provided.",
    "image_file": "https://example.com/image.jpg"
  },
  {
    "question_id": "writing_p2_q2",
    "question_number": 2,
    "question_text": "Write an essay based on the email provided.",
  ...
  return (
    <div className="exam-writing-p2-editor">
      {/* Toolbar */}
      <div className="exam-writing-p2-editor__toolbar">
        <div className="exam-writing-p2-editor__toolbar-left">
          <button
            className="exam-writing-p2-editor__btn exam-writing-p2-editor__btn--preview"
            onClick={() => setShowPreview(!showPreview)}
          >
            {showPreview ? "✏️ Edit Mode" : "👁️ Preview"}
          </button>
          <button
            className="exam-writing-p2-editor__btn exam-writing-p2-editor__btn--import"
            onClick={() => setShowImportJSON(!showImportJSON)}
          >
            📝 Import JSON
          </button>
        </div>
        <div className="exam-writing-p2-editor__toolbar-right">
          <span className="exam-writing-p2-editor__info">
            Part 2: Write an essay ({currentQuestions.length}/{MAX_QUESTIONS} questions)
          </span>
        </div>
      </div>
      ...
      {/* Import JSON Modal */}
      {showImportJSON && (
        <div className="exam-writing-p2-editor__import-modal">
          <div className="exam-writing-p2-editor__import-content">
            ...
            />
            {jsonError && (
              <div className="exam-writing-p2-editor__error">{jsonError}</div>
            )}
            <div className="exam-writing-p2-editor__import-actions">
      ...
      {/* Preview Mode */}
      {showPreview ? (
        <div className="exam-writing-p2-editor__preview">
          <div className="exam-writing-p2-editor__preview-header">
            <h3>Part 2 - TOEIC Writing</h3>
            <div className="exam-writing-p2-editor__preview-nav">
              {currentQuestions.map((q, idx) => (
                <button
                  key={q.id}
                  onClick={() => setCurrentIndex(idx)}
                  className={`exam-writing-p2-editor__preview-nav-btn ${
          ...
          <div className="exam-writing-p2-editor__preview-content">
            {/* Left: Question */}
            <div className="exam-writing-p2-editor__preview-question">
              <div className="exam-writing-p2-editor__preview-question-card">
                <h4>Question {currentQuestion.question_number}</h4>
                {currentQuestion.question_text && (
                  <div className="exam-writing-p2-editor__preview-text">
                    {currentQuestion.question_text}
                  </div>
                )}
                {currentQuestion.image_file && (
                  <div className="exam-writing-p2-editor__preview-image">
                    <img
            ...
            <div className="exam-writing-p2-editor__preview-answer">
              <div className="exam-writing-p2-editor__preview-answer-card">
                <div className="exam-writing-p2-editor__preview-answer-number">
                ...
                <textarea
                  ...
                  className="exam-writing-p2-editor__preview-notes"
                />
                <div className="exam-writing-p2-editor__preview-word-count">
        ...
        /* Edit Mode */
        <div className="exam-writing-p2-editor__edit">
          {/* Question Tabs */}
          <div className="exam-writing-p2-editor__tabs">
            {currentQuestions.map((q, idx) => (
              <button
                key={q.id}
                onClick={() => setCurrentIndex(idx)}
                className={`exam-writing-p2-editor__tab ${
                  currentIndex === idx ? "active" : ""
                }`}
              >
          ...
          {/* Question Form */}
          <div className="exam-writing-p2-editor__form">
            <div className="exam-writing-p2-editor__field">
              <label>
            ...
            <div className="exam-writing-p2-editor__field">
              ...
              <textarea
                ...
                rows={3}
                placeholder="Enter the question text (e.g., 'Write an essay based on the email provided.')..."
              />
            </div>
            ...
            <div className="exam-writing-p2-editor__field">
              ...
              />
              {currentQuestion.image_file && (
                <div className="exam-writing-p2-editor__image-preview">
                  <img
                    src={currentQuestion.image_file}
                    alt="Preview"
