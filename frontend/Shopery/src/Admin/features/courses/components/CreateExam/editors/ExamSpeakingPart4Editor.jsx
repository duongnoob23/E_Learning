// ExamSpeakingPart4Editor.jsx - Editor for Speaking Part 4 (Respond to questions using information provided)
// Part 4: 1 question required
// Features: Preview, JSON Import

import { useCallback, useEffect, useRef, useState } from "react";
import "./ExamSpeakingPart4Editor.css";

const MAX_QUESTIONS = 1;
const MIN_QUESTIONS = 1;

function createEmptyQuestion() {
  const id = `speaking_p4_q_${Date.now()}_${Math.random()
    .toString(36)
    .slice(2, 8)}`;
  return {
    id,
...
export default function ExamSpeakingPart4Editor({ questions = [], onChange }) {
  ...
  const mapExamDataToState = useCallback((examQuestions) => {
    ...
    return {
      id: q.question_id || q.id || `speaking_p4_q_${Date.now()}`,
      question_number: q.question_number || 1,
      question_text: q.question_text || "",
      image_file: q.image_file || "", // Optional
    };
  ...
  const handleImportJSON = () => {
    ...
    if (arr.length > MAX_QUESTIONS) {
      setJsonError(`Part 4 can only have ${MAX_QUESTIONS} question.`);
      return;
    }
    ...
    const mapped = {
      id:
        q.question_id ||
        `speaking_p4_q_${Date.now()}_${Math.random()
          .toString(36)
          .slice(2, 8)}`,
      question_number: q.question_number || 1,
      question_text: q.question_text || "",
  ...
  const sampleJSON = `[
  {
    "question_id": "speaking_p4_q1",
    "question_number": 1,
    "question_text": "Based on the information provided, what would you recommend?",
  ...
  return (
    <div className="exam-speaking-p4-editor">
      {/* Toolbar */}
      <div className="exam-speaking-p4-editor__toolbar">
        <div className="exam-speaking-p4-editor__toolbar-left">
          <button
            className="exam-speaking-p4-editor__btn exam-speaking-p4-editor__btn--preview"
            onClick={() => setShowPreview(!showPreview)}
          >
            {showPreview ? "✏️ Edit Mode" : "👁️ Preview"}
          </button>
          <button
            className="exam-speaking-p4-editor__btn exam-speaking-p4-editor__btn--import"
            onClick={() => setShowImportJSON(!showImportJSON)}
          >
            📝 Import JSON
          </button>
        </div>
        <div className="exam-speaking-p4-editor__toolbar-right">
          <span className="exam-speaking-p4-editor__info">
            Part 4: Respond to questions using information provided (1 question)
          </span>
        </div>
      </div>
      ...
      {/* Import JSON Modal */}
      {showImportJSON && (
        <div className="exam-speaking-p4-editor__import-modal">
          <div className="exam-speaking-p4-editor__import-content">
            ...
            />
            {jsonError && (
              <div className="exam-speaking-p4-editor__error">{jsonError}</div>
            )}
            <div className="exam-speaking-p4-editor__import-actions">
      ...
      {/* Preview Mode */}
      {showPreview ? (
        <div className="exam-speaking-p4-editor__preview">
          <div className="exam-speaking-p4-editor__preview-header">
            <h3>Part 4 - TOEIC Speaking</h3>
          </div>

          <div className="exam-speaking-p4-editor__preview-content">
            {/* Left: Question */}
            <div className="exam-speaking-p4-editor__preview-question">
              <div className="exam-speaking-p4-editor__preview-question-card">
                <h4>Question {currentQuestion.question_number}</h4>
                {currentQuestion.question_text && (
                  <div className="exam-speaking-p4-editor__preview-text">
                    {currentQuestion.question_text}
                  </div>
                )}
                {currentQuestion.audio_file && (
                  <div className="exam-speaking-p4-editor__preview-audio">
            ...
            <div className="exam-speaking-p4-editor__preview-answer">
              <div className="exam-speaking-p4-editor__preview-answer-card">
                <div className="exam-speaking-p4-editor__preview-answer-number">
                ...
                <textarea
                  ...
                  className="exam-speaking-p4-editor__preview-notes"
                />
                <button className="exam-speaking-p4-editor__preview-record-btn">
                  🎤 RECORD
                </button>
              </div>
            </div>
        ...
        /* Edit Mode */
        <div className="exam-speaking-p4-editor__edit">
          {/* Question Form */}
          <div className="exam-speaking-p4-editor__form">
            <div className="exam-speaking-p4-editor__field">
              <label>
