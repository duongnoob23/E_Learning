...
exports.vnpayReturn = async (req, res) => {
    ...
    try {
        ...
        if (!isValid) {
            ...
            return res.redirect(`${FRONTEND_URL}/payment/failed?message=Invalid+Checksum`);
        }
        ...
        if (result.success) {
            ...
            successUrl.searchParams.append("transactionId", result.transaction_id || "");

            return res.redirect(successUrl.toString());
        } else {
            // Thanh toán thất bại
            const failUrl = new URL(`${FRONTEND_URL}/payment/failed`);
        ...
        return res.redirect(`${FRONTEND_URL}/payment/failed?message=${encodeURIComponent(err.message || "Server Error")}`);
    }
};
